...
/*			cpu = SimulatorConstants.MIN_CPU 
			...
			MersenneTwister generator = new MersenneTwister(n);
			Uniform myUniformDist = new Uniform(generator);
			double util = myUniformDist.nextDoubleFromTo(0.3,0.6);
			//double util=1;
			node = new Server(nodeCount);
			// Random diskSpace generation
			...
			//System.out.println("aaaaaaaaa: " + util+ "  " +cpu);
			//cpu = SimulatorConstants.RACK_SERVER_CPU;
			...
			double trust = myUniformDist.nextDoubleFromTo(0.1,0.9);
			((Server) node).setTrust(trust);
		}
